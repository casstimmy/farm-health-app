...
/** Lazily create the transporter only when credentials are available */
    ...
    .map(
      (item, i) =>
        `<tr style="background:${i % 2 === 0 ? "#ffffff" : "#f9fafb"};">
          <td style="padding:10px 16px;border-bottom:1px solid #e5e7eb;font-size:14px;">${item.description || item.name || item.title || "Item"}</td>
...
<body style="margin:0;padding:0;background:#f3f4f6;font-family:'Segoe UI',Arial,sans-serif;">
  ...
  if (!transporter) {
    console.warn("Email not sent: EMAIL_USER / EMAIL_PASS environment variables are not configured.");
