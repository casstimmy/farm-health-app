  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div onClick={() => setFilterType(filterType === "Income" ? "all" : "Income")} className={`bg-green-50 border border-green-200 rounded-xl p-4 cursor-pointer hover:shadow-md transition-all active:scale-[0.98] ${filterType === "Income" ? "ring-2 ring-violet-500" : ""}`}><p className="text-sm text-gray-600">Total Income</p><p className="text-xl font-bold text-green-700">{hideAmounts ? "***" : formatCurrency(totalIncome, businessSettings.currency)}</p></div>
        <div onClick={() => setFilterType(filterType === "Expense" ? "all" : "Expense")} className={`bg-red-50 border border-red-200 rounded-xl p-4 cursor-pointer hover:shadow-md transition-all active:scale-[0.98] ${filterType === "Expense" ? "ring-2 ring-violet-500" : ""}`}><p className="text-sm text-gray-600">Total Expenses</p><p className="text-xl font-bold text-red-700">{hideAmounts ? "***" : formatCurrency(totalExpense, businessSettings.currency)}</p></div>
        <div onClick={() => setFilterType("all")} className={`border rounded-xl p-4 cursor-pointer hover:shadow-md transition-all active:scale-[0.98] ${netPL >= 0 ? "bg-blue-50 border-blue-200" : "bg-orange-50 border-orange-200"}`}><p className="text-sm text-gray-600">Net P&L</p><p className={`text-xl font-bold ${netPL >= 0 ? "text-blue-700" : "text-orange-700"}`}>{hideAmounts ? "***" : formatCurrency(netPL, businessSettings.currency)}</p></div>
                ...
                {filtered.map((r, idx) => (
                  <motion.tr key={r._id || idx} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: idx * 0.02 }} className="hover:bg-gray-50">
                    ...
                    <td className={`px-4 py-3 text-sm text-right font-bold ${r.type?.toLowerCase() === "income" ? "text-green-700" : "text-red-700"}`}>
                      {r.type?.toLowerCase() === "income" ? "+" : "-"}{hideAmounts ? "***" : formatCurrency(r.amount || 0, businessSettings.currency)}
                    </td>
