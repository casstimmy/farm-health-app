...
import { FaPlus, FaChevronDown, FaChevronUp, FaSpinner, FaTimes, FaTruck, FaCheckCircle, FaBoxOpen } from "react-icons/fa";
...
export default function OrdersPage() {
  ...
  const removeItem = (index) => {
    ...
    setAnimalSpeciesFilter((prev) => { const c = { ...prev }; delete c[index]; return c; });
  };
  ...
  const sendStatusEmail = useCallback(async (orderId, newStatus) => {
    try {
      const token = localStorage.getItem("token");
      await fetch("/api/orders/send-email", {
        ...
        body: JSON.stringify({
          orderId,
          status: newStatus,
          businessName: businessSettings?.businessName || "Farm Manager",
          currency: businessSettings?.currency || "NGN",
        }),
      });
    } catch (_) { /* email failure should not block */ }
  }, [businessSettings]);
  ...
  const speciesList = useMemo(() => {
    const set = new Set();
    animals.forEach((a) => { if (a.species) set.add(a.species); });
    return [...set].sort();
  }, [animals]);

  const getOrderCustomerName = (order) => {
    const cust = customers.find((c) => c._id === (order.customer?._id || order.customer));
    return customerDisplayName(cust || order.customer);
  };

  const filteredOrders = orders.filter((order) => {
    const matchesStatus = filterStatus === "all" || order.status === filterStatus;
    const term = searchTerm.toLowerCase().trim();
    const cName = getOrderCustomerName(order);
    const matchesSearch = !term || (order.orderNumber || "").toLowerCase().includes(term) || cName.toLowerCase().includes(term);
                  ...
                  <div className="p-5 space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                      <div>
                        ...
                        <select className="input-field" value={form.customer} onChange={(e) => setForm({ ...form, customer: e.target.value })}>
                          ...
                          {customers.map((c) => (
                            <option key={c._id} value={c._id}>{customerDisplayName(c)}</option>
                          ))}
                        </select>
                      </div>
                      <div>
                    ...
                    <div className="space-y-2">
                      ...
                      {(form.items || []).map((item, idx) => {
                        const speciesForIdx = animalSpeciesFilter[idx] || "";
                        const filteredAnimals = speciesForIdx ? animals.filter((a) => a.species === speciesForIdx) : animals;
                        return (
                                ...
                                <select className="input-field" value={item.description} onChange={(e) => {
                                  ...
                                  {filteredAnimals.map((a) => (
                                    <option key={a._id} value={a.tagId + " - " + (a.name || a.breed || "Animal")}>
                                      {a.tagId} - {a.name || a.breed || "Animal"} {a.species ? "(" + a.species + ")" : ""}
                                    </option>
                                  ))}
                                </select>
                              </div>
                  ...
                  {filteredOrders.map((order, idx) => {
                    ...
                    return (
                      <React.Fragment key={order._id}>
                        <tr className={(idx % 2 === 0 ? "bg-white" : "bg-gray-50") + " hover:bg-indigo-50 transition-colors"}>
                          ...
                          <td className="px-4 py-3 text-sm">{cName}</td>
                              ...
                              <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} transition={{ duration: 0.25, ease: "easeOut" }} className="overflow-hidden">
                                <div className="px-6 py-4">
                                  <div className="flex items-center justify-between mb-3">
                                    <h4 className="text-sm font-bold text-indigo-800">Order {order.orderNumber} &mdash; Details</h4>
                                  ...
                                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
                                    <div className="bg-white rounded-lg p-3 border border-indigo-100">
                                      ...
                                      <p className="text-sm font-bold">{cName}</p>
                                      {(() => { const cu = customers.find((c) => c._id === (order.customer?._id || order.customer)); const em = cu?.email || order.customerEmail || ""; return em ? <p className="text-xs text-gray-500">{em}</p> : null; })()}
                                    </div>
                                    <div className="bg-white rounded-lg p-3 border border-indigo-100">
